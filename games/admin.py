...
from .models import TimeCategory, TimeRiddle, TimeGameProgress, TimePlayHistory, TimeSessionPackage
    ...
    def clean(self):
        ...
        for form in self.forms:
            ...
            if empty_extra:
                continue
            alive += 1
        pkg = self.instance  # GamePackage (Proxy: TimePackage)
        limit = 40  # حد كل حزمة
        if alive > limit:
            raise forms.ValidationError(f"الحدّ الأقصى لهذه الحزمة هو {limit} صورة/لغز.")
...
class TimePackageAdmin(admin.ModelAdmin):
    ...
    inlines = [TimeRiddleInline]  # 👈 إضافة عناصر الحزمة من نفس الصفحة
...
class TimeRiddleAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        qs = super().get_queryset(request)
        return qs.select_related('package', 'package__time_category').filter(package__game_type='time')
